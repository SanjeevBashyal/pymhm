...
_SPECS = {
    "lc": {
        "combo": "mMapLayerComboBox_land_cover",
        "label": "Land Cover",
    ...
    "geology": {
        "combo": "mMapLayerComboBox_geology",
        "label": "Geology",
...
class CategoricalProcessingMixin(
    ...
    def _process_categorical(self, kind, write_classdefinition=True):
        ...
        try:
            with TemporaryDirectory(prefix=f"pymhm_{kind}_", dir=geometry) as temp:
                temporary = Path(temp)
