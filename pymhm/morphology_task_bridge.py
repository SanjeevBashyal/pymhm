...
def _run_in_worker(task, payload, working_folder):
    """Run one job in `pymhm.native_worker` so a native failure cannot kill QGIS."""
    if task.isCanceled():
        raise RuntimeError("Task cancelled.")
    task.setProgress(5)
    with TemporaryDirectory(
            prefix="pymhm_native_", dir=str(working_folder)) as temporary:
...
class MorphologyTaskBridge(QtCore.QObject):
    ...
    def _dem_options(self):
        ...
        layer = self.dialog.mMapLayerComboBox_dem.currentLayer()
    ...
    def _lookup_job(self, kind):
        ...
        if local is None:
            ...
            if candidate.is_file():
                local = str(candidate)
        if local is None:
            with TemporaryDirectory(prefix=f"pymhm_{kind}_boundary_") as temporary:
    ...
    def _finish_execute_all(self):
        try:
            if self.dialog.mMapLayerComboBox_pour_points.currentLayer() is not None:
