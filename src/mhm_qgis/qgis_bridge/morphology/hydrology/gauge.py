...
class GaugePositionMixin(PourPointWorkflowMixin, PredecessorMixin):
    """Gauge-position raster preparation from snapped pour points."""
    ...
    def process_gauge_position(self) -> None:
        ...
        try:
            ...
            domain_state = load_domain_state(self.session.project_folder)
            configured_points = {
            ...
            self.error("Invalid Outlet IDs", str(e))
            self.log_message(f"ERROR: {e}")
            return

        if hasattr(self.dialog, "update_gauged_outlet_count"):
            self.dialog.update_gauged_outlet_count()
        ...
        for item in gauge_features:
            ...
            if source_crs is not None and source_crs.isValid():
                transform = layers.transform_between(source_crs, filled_dem_crs)
