...
class LaiProcessingMixin(
    ...
    def process_lai_long_term_monthly_netcdf(self) -> bool:
        ...
        if source is None:
            self.warn("Input Error", "Select a LAI NetCDF file.")
            return False
        ...
        filled_dem_layer = layers.open_layer(self.filled_dem_path, "Filled_DEM")
        if filled_dem_layer is None:
        ...
        output_path = lai_dem_staging_path(self.session.project_folder)
    ...
    def crop_lai_netcdf_to_l0(self, l0_header: dict, input_crs) -> bool:
        ...
        staged_path = lai_dem_staging_path(self.session.project_folder)
        ...
        output_folder = lai_folder(self.session.project_folder)
    ...
    def _selected_lai_layer(self):
        ...
        source = self.session.source_for("lai")
        if not source:
            return None
        from ... import layers

        return layers.open_layer(source, "LAI")
    ...
    def lai_task_options(self, output_path: str) -> dict | None:
        ...
        return {
            ...
            "dem_crs": self._qgis_crs_to_pyproj(self.session.crs) or "",
            "target_timestep": (
    ...
    def _write_resampled_lai_netcdf(
        ...
        prepare_lai_file(
            ...
            dem_crs=self._qgis_crs_to_pyproj(self.session.crs),
            log=self.log_message,
        )
        ...
        update_section(
            ...
            "lai",
            {
                ...
                "output_path": relative_workspace_path(
                    self.session.project_folder, output_path
                ),
                "variable": "lai",
            },
        )
