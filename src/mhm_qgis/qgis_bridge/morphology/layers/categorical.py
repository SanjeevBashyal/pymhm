...
from ..common import (QgsVectorLayer,
...
from ....core.morphology.soil_sources import local_layer_source, materialize_vector_layer
...
class CategoricalProcessingMixin(
    ...
    def _process_categorical(self, kind, write_classdefinition=True):
        ...
        geometry = Path(project_geometry_folder(self.session.project_folder))
        staging = Path(morph_staging_folder(self.session.project_folder))
    ...
    def _categorical_input(self, kind, spec):
        source = self.session.source_for(spec["kind"])
        layer = None
        if not source:
    ...
    def _copy_ready_categorical(self, kind, source, is_vector):
        ...
        target = (
            Path(morph_folder(self.session.project_folder))
    ...
    def _record_categorical_nml(self, kind, output, definition=None):
        ...
        value = {
            "mode": self._categorical_mode(kind),
            "output_path": relative_workspace_path(self.session.project_folder, output),
            "variable": {
                "lc": "land_cover",
                "soil": "soil_class",
        ...
        if definition is not None and Path(definition).is_file():
            value["classdefinition_path"] = relative_workspace_path(
                self.session.project_folder, definition
            )
        config = self._categorical_lookup(kind) or {}
        for key in ("lookup_table", "mapping_field", "class_field"):
            if config.get(key):
                value[key] = config[key]
        update_section(self.session.project_folder, section, value)

    def _categorical_intermediate_paths(self, kind):
        base = (
            Path(project_geometry_folder(self.session.project_folder))
    ...
    def _ready_output_paths(self, kind):
        spec = _SPECS[kind]
        folder = Path(morph_folder(self.session.project_folder))
        paths = [
