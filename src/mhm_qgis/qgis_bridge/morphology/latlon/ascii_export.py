...
from ....core.handlers.file.ascii.morphology import (
...
class AsciiExportMixin(MaskingMixin):
    ...
    def write_all_layers(self, show_error_dialog=True) -> bool:
        ...
        try:
            headers = self.session.grid_headers
        ...
        geom_folder = geometry_folder(self.session.project_folder)
        ...
        morph_output_folder = morph_folder(self.session.project_folder)
        os.makedirs(morph_output_folder, exist_ok=True)

        if is_v6(self.session.project_folder):
    ...
    def _write_morph_input_nc(
        ...
        from ....core.morphology.layers.morph_input_nc import (
            available_layers,
            write_morph_input_nc,
        )

        geometry = geometry_folder(self.session.project_folder)
        ...
        if not layers:
            ...
            if show_error_dialog:
                self.warn("No Masked Layers", message)
            return False
        ...
        try:
            ...
            if show_error_dialog:
                self.warn("input.nc Error", str(error))
            return False
