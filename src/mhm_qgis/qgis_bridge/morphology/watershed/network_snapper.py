...
class NetworkSnapperMixin(VectorIOMixin):
    ...
    def snap_points_to_network(
        ...
        if order_field_name not in channel_network_layer.fields().names():
            ...
            self.error("Missing Field", f"Could not find order field '{order_field_name}' in the channel network layer.")
            return None
        ...
        if not output_crs.isValid():
            output_crs = self.session.crs

        pour_points_crs = pour_points_layer.crs()
        point_transform = layers.transform_between(pour_points_crs, output_crs)
