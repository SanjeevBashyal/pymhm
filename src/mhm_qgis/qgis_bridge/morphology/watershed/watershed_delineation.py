...
class WatershedDelineationMixin(PourPointWorkflowMixin):
    ...
    def delineate_watershed(self) -> None:
        ...
        snapped_points_layer = layers.open_layer(
            self.snapped_points_path, "Snapped Points", is_raster=False)
        if snapped_points_layer is None:
            self.warn("Error", "Could not load snapped points layer."
            )
            return

        features = list(snapped_points_layer.getFeatures())
        if not features:
            self.warn("Error", "No snapped points found.")
            return
        ...
        if not watershed_outputs:
            self.warn("Error", "No watersheds were successfully created."
            )
            return
    ...
    def delineate_single_outlet(
        ...
        if not np.isfinite(catchment_area_m2) or catchment_area_m2 <= 0:
            catchment_area_m2 = (
                ...
                * self._reference_cell_area_m2(reference)
            )
