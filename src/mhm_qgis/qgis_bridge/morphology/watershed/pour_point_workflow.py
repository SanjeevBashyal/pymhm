...
class PourPointWorkflowMixin(
    ...
    def snap_points(self) -> None:
        ...
        channel_network_layer = layers.open_layer(
            self.channel_network_vector_path, "Channel Network", is_raster=False)
        if channel_network_layer is None:
            self.log_message(
