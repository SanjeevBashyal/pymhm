...
class PredecessorMixin(BaseProcessingMixin, NamingAndRangeMixin):
    ...
    def _filled_dem_matches_current_input(self, path: str) -> bool:
        ...
        dem_layer = self.dialog.input_combo("dem").currentLayer()
        if not dem_layer:
            return False
    ...
    def _vector_crs_matches_raster(self, vector_path: str, raster_path: str) -> bool:
        ...
        vector_layer = layers.open_layer(vector_path, "Existing_Vector", is_raster=False)
        raster_layer = layers.open_layer(raster_path, "Reference_Raster")
        if vector_layer is None or raster_layer is None:
            return False
    ...
    def _snapped_points_match_channel_network(
        ...
        snapped_layer = layers.open_layer(
            snapped_path, "Existing_Snapped_Points", is_raster=False)
        channel_layer = layers.open_layer(
            channel_network_path, "Existing_Channel_Network", is_raster=False)
        if snapped_layer is None or channel_layer is None:
            return False
