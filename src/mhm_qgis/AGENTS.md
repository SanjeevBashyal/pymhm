...
Guidance for coding agents working on the `mhm_qgis` QGIS plugin.

## Project Shape

`mhm_qgis` is a QGIS 3 plugin that prepares mHM model inputs, namelists, and supporting morphology/meteorology data.

Key entry points:

- `__init__.py`: QGIS plugin factory.
- `mhm_qgis.py`: plugin lifecycle, toolbar/menu action, reload-safe import handling.
- `mhm_qgis_dialog.py`: dialog wiring, signal connections, UI state persistence.
- `ui_mhm_qgis_dialog_base.py` and `mhm_qgis_dialog_base.ui`: generated/base UI files. Avoid touching generated UI unless the task is explicitly UI-related.
...
- Import the installed `mhm_tools` package directly. This repository does not
  bundle a second `src/mhm_qgis/mhm_tools` source tree.
...
- ERA5-Land temperature files are read once for `tavg`, `tmin`, and `tmax`
  ...
  `MHM_QGIS_METEO_MAX_BYTES` (2 GiB default). A long record needs all three series
...
- `Meteorology/reuse.py` skips meteorology preparation when every required
  variable is recorded in `mhm_qgis_processing_state.json`, still on disk, and its
...
- The validated L0 header, L2 header, and multiplier are mirrored into the
  `grid` section of `mhm_qgis_processing_state.json` for resumes.
...
- **Domain DEMs are written during Morphology Setup, not at delineation.**
  ...
  in `mhm_qgis_processing_state.json` under `domains`; step 6/6
...
- `project_layout.project_version()` reads `mhm_version` from
  `mhm_qgis_input_state.json`, so the path helpers branch on version internally and
...
- `input.nc` carries seven variables: `dem, fdir, slope, aspect, geology_class,
  soil_class, facc`. **`LAI_class` is deliberately absent** -- mhm_qgis has no LAI
...
- `state_cache.py` records, in `mhm_qgis_processing_state.json`, a fingerprint of
...
- Infer the input cadence from the NetCDF time coordinate; do not ask the user
...
- The Execute All Rasterio warp fills cells the source cannot supply with
  **0**: a missing leaf area is none, not a gap. The later aligned window copy
  pads beyond the staged extent with the same **0**. LAI is not
  watershed-masked, so `-9999` should not appear in a published LAI cube.
...
- Reusable temporal conversion, Rasterio warping, streaming NetCDF writing, and
  aligned window copying live in `mhm_tools.pre.format_lai`. Call that public
  API only through `mhm_tools_adapter.py`. `lai.py` remains the thin QGIS
  boundary, and `lai_task_options()` snapshots paths and primitive values for
  the worker. Never call a processor method or touch a widget from the worker.
- mHM-tools streams every stage one block of target rows and one time step at a
  time. Never materialise the whole target cube or the full WGS84 target mesh.
- `lai_window_offsets()` validates cell size and alignment before copying and
  raises rather than silently shifting the data.
- Target WGS84 coordinates are generated by row block rather than as a full
  mesh; projected grids must remain memory-bounded too.
...
- `assert_lai_output_fits()` is a disk check, not a memory check: memory is
  bounded by the writer. `MHM_TOOLS_LAI_MAX_BYTES` overrides it.
...
- `mhm_qgis_domain_delineation_state.json` remains canonical. Project domain
...
- Useful low-risk checks:
  ...
  - Targeted render smoke tests for `Configuration/namelist.py` and `Configuration/version_compat.py` using a lightweight fake `mhm_qgis` package import harness.
...
- QGIS reload can import `mhm_qgis.py` as top-level `mhm_qgis`; keep reload-safe import handling intact.
...
- **All heavy categorical formatting runs in `mhm_qgis.native_worker`**, a child
...
- `categorical_lookup.py` and `geology_metadata.py` sit at the top level, not
  under `src/mhm_qgis/Morphology/`, because importing anything from that package runs
